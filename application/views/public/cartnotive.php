            ...
            <?php
                if (!empty($keranjang) || !empty($custom)) {
                    ...
                    <span class="heartbit"></span>
                     ...
                     right: 22px;
                     top: -5px;
                     font-size: small;">'.(count($keranjang)+count($custom)).'</span>
                    </div>';
                }
            ?>
        </a>
                            ...
                            <?php
                                ...
                                if(!empty($custom[0]->id_custom)){
                                    ...
                                    foreach($custom as $key => $value){
                                        $harga_satuan = ($value->harga_satuan*$value->panjang*$value->lebar*$value->tinggi);
                                        $img = (empty($profil->images) ? 'noimages.jpg': 'noimages.jpg'); 
                                        ?>
                                        <a href="<?=base_url('public/home/cart')?>"
                                            class="message-item d-flex align-items-center border-bottom px-3 py-2"
                                            id='c<?=$value->id_detail_produk?>' data-user-id='<?=$value->id_detail_produk?>'>
                                            <span class="user-img position-relative d-inline-block"> 
                                                <img src="<?=base_url('images/').$img?>" alt="user" class="w-100">
                                                <span class="profile-status rounded-circle offline"></span> </span>
                                            <div class="w-75 d-inline-block v-middle ps-3">
                                                <h5 class="message-title mb-0 mt-1"><?="Order Custom"?></h5> <span
                                                    class="fs-2 text-nowrap d-block text-muted text-truncate">Qt :<?=$value->jumlah?> Pcs</span>
                                                    <span class="fs-2 text-nowrap d-block text-muted"><?=rp($value->jumlah*$harga_satuan);?></span> 
                                            </div>
                                        </a>
                                        <?php
                                    }
                                        ...
                                        $imgcek = file_exists(FCPATH.'images/'.$value->images) ? $value->images : 'noimages.jpg';
                                            ...
                                            <a href="<?=base_url('public/home/cart')?>"
                                                ...
                                                <span class="user-img position-relative d-inline-block"> 
                                                    ...
                                                    <span class="profile-status rounded-circle offline"></span> </span>
                                                <div class="w-75 d-inline-block v-middle ps-3">
                                                    <h5 class="message-title mb-0 mt-1"><?=$value->nama_produk?></h5> <span
                                                        class="fs-2 text-nowrap d-block text-muted text-truncate">Qt :<?=$value->jumlah?> Pcs</span> 
