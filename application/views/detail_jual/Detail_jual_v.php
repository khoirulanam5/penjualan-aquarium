              ...
              <div class="card-body">
               ...
               <?php  
                  if($this->input->get("act")=="view"){
                      $view = $this->db->get_where("Detail_jual",array("id_Detail_jual"=>$this->input->get("id_Detail_jual")))->result();
                      $act = "Update";
                  ...
                  if(!empty($view)){
                    ?>
                     <form action="<?=base_url()?>Detail_jual/Detail_jual/<?=$act?>" method="post">
                      <div class="modal-body">
                          <div class="table_isi">
                              <div class="form-group"><label for="inputEstimatedBudget">Nama</label>
                                  <input name="id_Detail_jual" type="hidden"  value="<?=$view[0]->id_Detail_jual?>" >
                                  ...
                                  <tbody>
                                      <?php
                                          ...
                                          if (!empty($datas)) {
                                              ...
                                              ?>
                                              <tr id="<?= $value->id_Detail_jual ?>">
                                                  <?php
                                                      echo "<th>" . strtoupper($value->id_Detail_jual) . "</th>";
                                                  ...
                                                  ?>
                                                  <td>
                                                      <button class="btn btn-xs btn-primary" href="" id="<?= $value->id_Detail_jual ?>" onclick="action_view("<?= $value->id_Detail_jual ?>")"><i class="fa fa-eye"></i></button>
                                                      <button class="btn btn-xs btn-primary" onclick="action_delete("<?= $value->id_Detail_jual ?>")"><i class="fa fa-trash-alt"></i></button>
                                                  </td>
                                              </tr>
                                              <?php $no++;
                                                  }
...
const action_view = (id) => {
    ...
    console.log(data);
    location.href = "?act=view&id_Detail_jual=" + data[0];
}
...
const action_delete = (id) => {
    ...
    $.ajax({
        url: "<?=base_url()?>Detail_jual/Detail_jual/delete",
        type: "POST",
        data: {
            row
        },
